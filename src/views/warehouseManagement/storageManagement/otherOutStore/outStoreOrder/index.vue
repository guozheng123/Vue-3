            ...
            <LoadingSkeleton>
                <TpfTableLayout showChangeTableFontSize showChangeTableGap :pageCode="pageCode">
                    ...
                    <template #operateLeft>
                        ...
                        <a-button ghost disabled type="primary" class="tpf-button">
...
<script setup lang="tsx" name="outStorageOrder">
    ...
    const pageCode = PageCodeEnum.warehouseManagementOtherOutTable;
    ...
    const onAdd = () => {
        router.push({
            path: '/warehouseManagement/storageManagement/otherOutStore/outStoreOrderOrderEditDetail'
        });
    };
    const onSubmit = (val: any) => {
        val.mainEntityCode = 't_other_out_in_store';
    ...
    const beforeRender = (list: TpfColumnType[]) => {
        return unref(list).map(item => {
            ...
            if (item.dataIndex === 'out_no') {
                item.customRender = ({ text, record }) => (
                    <span
                        class="tpf-link"
                        onClick={() => {
                            console.log(record.id, 998);
                            router.push({
                                path: '/warehouseManagement/storageManagement/otherOutStore/outStoreOrderOrderEditDetail',
