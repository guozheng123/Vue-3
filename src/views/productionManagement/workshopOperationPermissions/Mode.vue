...
<script setup lang="ts" name="WorknModel">
    ...
    import { userReportManage } from '@/api';
    ...
    const { tableInfo, pagination, loading, getDataList, selectedRowInfo } = useAntdTable({
        api: async pageInfo => {
            ...
            const res = await userReportManageAPi.queryOperationUserList(unref(params));
    ...
    const onSave = () => {
        const params = selectedRowInfo.value.selectedRows.map((e: any) => {
            const list = {
                userId: e.userId,
