...
import TpfDelConfirm from '@/components/TpfModal/TpfDelConfirm';
...
export const useUserStore = defineStore({
        ...
        strategies: [
            {
                ...
                paths: ['loginInfo', 'userInfo', 'visionInfo']
            }
        ]
    },
    state: (): UserState => ({
    ...
    actions: {
        ...
        async afterLogin() {
            const {
                // VITE_APP_USE_GATEWAY = '',
                VITE_APP_API_ROOT = '',
                VITE_APP_USE_LOCAL_ROUTE = false
            } = import.meta.env;
            let data: any[] = [];
            if (VITE_APP_USE_LOCAL_ROUTE === 'true') {
                data = routerJson;
            } else {
        ...
        async getVisionInfo() {
            const res = await http.get('/checkUpdate.json');
            const visionInfo = this.visionInfo || '';
            const info = JSON.stringify(res);
            this.visionInfo = info;
            if (visionInfo && visionInfo !== info) {
                console.log('visionInfo: ', visionInfo);
                try {
                    await TpfDelConfirm({
                        title: '提示',
                        content: '存在新的版本发布,请刷新浏览器'
                    });
                } finally {
                    location.reload();
                }
