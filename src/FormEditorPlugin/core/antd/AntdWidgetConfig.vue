<template>
    <a-form layout="vertical" v-if="data" :key="data.key">
        <a-form-item label="字段标识" v-if="data.type !== 'grid'">
            <a-select ref="select" v-model:value="data.model" @change="changeModel">
                <a-select-option
                    ...
                    <a-radio-group v-model:value="data.options.defaultValue">
                        ...
                        >
                            <template #item="{ element, index }">
                                <div>
                                    <a-radio :value="element.value" style="margin-right: 0">
                                        <a-input
                                            size="small"
                                            :style="{
        ...
        <template v-if="data.type !== 'grid'">
            <a-form-item
                ...
                "
            >
                <a-checkbox v-if="hasKey('rules')" v-model:checked="data.options.rules.required"
                    >必填</a-checkbox
                >
                <a-checkbox v-if="hasKey('readonly')" v-model:checked="data.options.readonly"
                    >只读</a-checkbox
                >
                <a-checkbox v-if="hasKey('disabled')" v-model:checked="data.options.disabled"
                    >禁用</a-checkbox
                >
                <a-checkbox v-if="hasKey('allowClear')" v-model:checked="data.options.allowClear"
                    >清除</a-checkbox
                >
            </a-form-item>
            ...
            <template v-if="hasKey('rules')">
                ...
                <a-form-item label="触发时机">
                    <a-radio-group v-model:value="data.options.rules.trigger" button-style="solid">
                ...
                <a-form-item label="枚举类型" v-if="0">
                    <a-input v-model:value="data.options.rules.enum" />
                </a-form-item>

                <a-form-item label="字段长度" v-if="0">
...
<script lang="ts" setup name="AntdWidgetConfig">
    import { ref, watch } from 'vue';
    import Draggable from 'vuedraggable';
    import SvgIcon from '@/FormEditorPlugin/components/SvgIcon.vue';
    import { SelectValue } from 'ant-design-vue/lib/select';
    const select1 = [
        { value: 'k1', title: '输入框字段1' },
        { value: 'k2', title: '输入框字段2' },
        { value: 'k3', title: '输入框字段3' }
    ];
    const select2 = [
        { value: 's1', title: '输下拉框字段1' },
        { value: 's2', title: '输下拉框字段2' },
        { value: 's2', title: '输下拉框字段3' }
    ];
    // defineOptions({
    ...
    const emit = defineEmits<{
        (e: 'update:select', val: typeof props.select): void;
    }>();
    ...
    watch(
        ...
        val => {
            ...
            data.value = val;
            if (val.type === 'input') {
                selectOptions.value = select1;
            }
            if (val.type === 'select') {
                selectOptions.value = select2;
            }
        }
    );

    watch(data, val => emit('update:select', val));
    const changeModel = (val: SelectValue) => {
        const label = (selectOptions.value.find(item => item.value === val) || {}).title;
    ...
    const handleSliderModeChange = (checked: boolean | string | number) => {
        checked
